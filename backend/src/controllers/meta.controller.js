  ...
  try {
    ...
    const authUrl = axios.getUri({
      ...
      params: {
        ...
        scope:
          "ads_management,ads_read,business_management,leads_retrieval,pages_show_list,pages_read_engagement",
        state,
      },
    });
...
const metaAuthCallback = async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn("Long-lived token exchange warning:", exchangeErr.message);
    }
    ...
    try {
      ...
      console.log(`[OAuth Successful] || "None found"}`);
    } catch (pixelErr) {
      console.warn("Failed to fetch pixels during OAuth:", pixelErr.message);
    }
...
const getPageForms = async (req, res, next) => {
  try {
    ...
    // Inside getPageForms (Replace the extractedForms logic)
    const existingForms = pageRecord.forms || [];

    const extractedForms = formsData.data.map((form) => {
      // Find if this form already exists in DB to keep its cron state
      const existing = existingForms.find((f) => f.formId === form.id);
      return {
        formId: form.id,
        name: form.name,
        isCronActive: existing ? existing.isCronActive : false,
      };
    });
  ...
  try {
    ...
    for (const fbLead of leadsFromMeta.data) {
      ...
      const leadData = {
        ...
        source: "META",
        status: "NEW",
        metaCreatedAt: originalTimestamp,
      };
      ...
      for (const change of entry.changes || []) {
        if (change.field === "leadgen") {
          ...
          if (result.created) {
            ...
            console.log(`[Webhook] Lead ${leadgenId} already existed`);
          }
        }
      }
    }
...
// Background function that runs every 5 minutes without needing an HTTP request
const performAutoSync = async (formId) => {
  try {
    // Find the page containing this form
    const pageRecord = await Page.findOne({ "forms.formId": formId });
    if (!pageRecord || !pageRecord.accessToken) return;

    const leadsFromMeta = await metaService.getFormLeads(
      formId,
      pageRecord.accessToken,
    );
    if (!leadsFromMeta || !leadsFromMeta.data) return;
    ...
    for (const fbLead of leadsFromMeta.data) {
      ...
      const leadData = {
        ...
        name:
          ...
          getFieldValue("first_name") ||
          "Unknown",
        email: getFieldValue("email") || "no-email@provided.com",
    ...
    console.log(`[CRON] Auto-synced leads for form: ${formId}`);
  } catch (error) {
    console.error(`[CRON Error] Failed to sync form ${formId}:`, error.message);
  }
};

const enableFormCron = async (req, res, next) => {
  try {
    const { formId } = req.params;

    // Stop existing job if one is already running for this form
    if (activeCronJobs.has(formId)) {
      activeCronJobs.get(formId).stop();
    }

    const job = cron.schedule("*/10 * * * * *", () => {
      performAutoSync(formId);
    });

    activeCronJobs.set(formId, job);

    await Page.findOneAndUpdate(
      { "forms.formId": formId },
      { $set: { "forms.$.isCronActive": true } },
    );

    res
      .status(200)
      .json({ success: true, message: "Cron job enabled for 5 minutes" });
  } catch (error) {
    next(error);
  }
};

const disableFormCron = async (req, res, next) => {
  try {
    const { formId } = req.params;
    ...
    if (activeCronJobs.has(formId)) {
      activeCronJobs.get(formId).stop();
      activeCronJobs.delete(formId);
    }

    await Page.findOneAndUpdate(
      { "forms.formId": formId },
      { $set: { "forms.$.isCronActive": false } },
    );

    res.status(200).json({ success: true, message: "Cron job disabled" });
    console.log(`[CRON] Disabled auto-sync for form: ${formId}`);
  } catch (error) {
    next(error);
  }
};
